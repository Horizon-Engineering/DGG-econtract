...
public class EditTextContract extends EditText {

	// The image we are going to use for the Clear button
	private Drawable imgCloseButton = getResources().getDrawable(R.drawable.clear_et);

	public EditTextContract(Context context) {
		super(context);
		init();
	}

	public EditTextContract(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public EditTextContract(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	...
	void init() {
		...
		int paddingTop = (int) getResources().getDimension(R.dimen.et_padding_top_con);
		int paddingBottom = (int) getResources().getDimension(R.dimen.et_padding_bottom_con);
		...
		this.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				EditTextContract et = EditTextContract.this;
				...
				if (event.getX() > et.getWidth() - et.getPaddingRight() - imgCloseButton.getIntrinsicWidth()) {
					et.setText("");
					EditTextContract.this.handleClearButton();
				}
				return false;
			}
		});
