...
public class InkPageIndicator extends View implements ViewPager.OnPageChangeListener, View.OnAttachStateChangeListener {
	...
	private static final int DEFAULT_UNSELECTED_COLOUR = 0x80ffffff; // 50%
																		// white
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		if (isAttachedToWindow) {
			...
			if (currentPosition != position) {
				...
				// if user scrolls completely to next page then the position
				// param updates to that
				// new page but we're not ready to switch our 'current' page yet
				// so adjust for that
				if (fraction == 1f) {
	 ...
	 *
	 * #1 At rest #2 Joining neighbour, still separate #3 Joining neighbour,
	 * combined curved #4 Joining neighbour, combined straight #5 Join
	 * retreating #6 Dot re-showing / revealing
	 *
	 * It can also be in a combination of these states e.g. joining one
	 * neighbour while retreating from another. We therefore create a Path so
	 * that we can examine each dot pair separately and later take the union for
	 * these cases.
	 *
	...
	private Path getUnselectedPath(int page, float centerX, float nextCenterX, float joiningFraction,
		...
		if (joiningFraction > 0.5f && joiningFraction < 1f && retreatingJoinX1 == INVALID_FRACTION) {
			...
			// adjust the fraction so that it goes from 0.3 -> 1 to produce a
			// more realistic 'join'
		...
		if (joiningFraction == 1 && retreatingJoinX1 == INVALID_FRACTION) {
			...
			// case #4 Joining neighbour, combined straight technically we could
			// use case 3 for this
			// situation as well but assume that this is an optimization rather
			// than faffing around
	...
	private void setSelectedPage(int now) {
		...
		// create the anim to move the selected dot  this animator will kick
		// off
	...
	private ValueAnimator createMoveSelectedAnimator(final float moveTo, int was, int now, int steps) {
		...
		moveSelected.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationStart(Animator animation) {
				// set a flag so that we continue to draw the unselected dot in
				// the target position
			...
			@Override
			public void onAnimationEnd(Animator animation) {
				// set a flag when anim finishes so that we don't draw both
				// selected & unselected
				// page dots
				selectedDotInPosition = true;
			}
	 ...
	 * reveals  to be started when the retreat has passed the dot to be
	 * revealed.
	 */
	public class PendingRetreatAnimator extends PendingStartAnimator {
		...
		public PendingRetreatAnimator(int was, int now, int steps, StartPredicate predicate) {
			...
			// direction we're
			// travelling in. Also look at the current selected dot position,
			// i.e. we're moving on
			...
			// retreat so that
			// we can initialize their revealFraction's i.e. make sure they're
			// hidden while the
			...
			if (initialX1 != finalX1) { // rightward retreat
				...
				// create the reveal animations that will run when the retreat
				// passes them
				for (int i = 0; i < steps; i++) {
			...
			addListener(new AnimatorListenerAdapter() {
				...
				public void onAnimationStart(Animator animation) {
					...
					// we need to set this so that the dots are hidden until the
					// reveal anim runs
	...
	/**
	 * A predicate used to start an animation when a given value is greater than
	 * a threshold
	 */
